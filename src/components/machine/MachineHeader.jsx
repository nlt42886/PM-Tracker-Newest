...
export default function MachineHeader({ machine }) {
  const { name, model, manufacturer, serialNumber, location, notes, logo } = machine;

  return (
    <div className="bg-white border border-slate-200 rounded-lg shadow-sm p-5 mb-4">
      <div className="flex flex-col sm:flex-row sm:items-start gap-4">
        <div className="bg-slate-100 rounded-lg flex items-center justify-center shrink-0 w-28 h-20">
          {logo ? (
            <img
              src={logo}
              alt={manufacturer || name}
              className="max-h-14 max-w-[96px] w-auto object-contain"
            />
          ) : (
            <svg
